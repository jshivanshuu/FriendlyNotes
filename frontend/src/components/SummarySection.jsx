...
const SummarySection = () => {
  ...
  const summarizeFile = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.detail || 'Summary failed');
      }
  ...
  return (
    <div className="upload-container" style={{ width: '100%' }}>
      {summary ? (
        <div className="glass-panel summary-view">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <h2 style={{ color: 'var(--accent-color)' }}>PDF Summary</h2>
            <button 
              className="btn" 
              style={{ padding: '0.5rem 1rem', background: 'transparent', border: '1px solid var(--accent-color)', color: 'var(--accent-color)' }}
              onClick={() => setSummary('')}
            >
        ...
        <div className="glass-panel">
          ...
          <div style={{ marginTop: '2rem' }}>
            <button 
              ...
              style={{ width: '100%' }}
            >
              {isUploading ? 'Summarizing (this may take a minute)...' : 'Generate Summary'}
            </button>
          </div>
        </div>
      )}
